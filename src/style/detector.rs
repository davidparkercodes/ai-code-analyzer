...
impl StyleDetector {
    ...
    fn process_directory(
        &self,
        dir_path: &Path,
        analysis: &mut CodeStyleAnalysis,
    ) -> Result<(), String> {
    ...
    fn process_file(
        ...
        if let Some(file_path_str) = file_path.to_str() {
            analysis
                .file_profiles
                .insert(file_path_str.to_string(), style_profile);
        }
    ...
    fn detect_indentation(&self, content: &str) -> IndentationType {
        ...
        let (most_common, _) = space_counts
            .iter()
    ...
    fn analyze_rust_functions(&self, content: &str) -> FunctionMetrics {
        ...
        let avg_params =
            functions.iter().map(|(_, p)| *p).sum::<usize>() as f64 / functions.len() as f64;
    ...
    fn analyze_naming_conventions(
        ...
        content: &str,
        language: &str,
    ) -> HashMap<String, NamingConvention> {
        ...
        match language {
            "Rust" => {
                ...
                result.insert(
                    "variables".to_string(),
                    self.detect_variable_convention(content, language),
                );

                // Functions
                let fn_pattern = Regex::new(r"fn\s+(\w+)").unwrap();
                let function_names: Vec<String> = fn_pattern
                ...
                if !function_names.is_empty() {
                    result.insert(
                        "functions".to_string(),
                        self.determine_naming_convention(&function_names),
                    );
                }
                ...
                if !type_names.is_empty() {
                    result.insert(
                        "types".to_string(),
                        self.determine_naming_convention(&type_names),
                    );
                }
            }
            "JavaScript" | "TypeScript" => {
                ...
                if language == "TypeScript" {
                    ...
                    let class_names: Vec<String> = class_pattern
                        .captures_iter(content)
                        .filter_map(|cap| cap.get(1).map(|m| m.as_str().to_string()))
                        .collect();
                    ...
                    if !class_names.is_empty() {
                        result.insert(
                            "classes".to_string(),
                            self.determine_naming_convention(&class_names),
                        );
                    }

                    // Interfaces
                    let interface_pattern = Regex::new(r"interface\s+(\w+)").unwrap();
                    let interface_names: Vec<String> = interface_pattern
                        .captures_iter(content)
                        .filter_map(|cap| cap.get(1).map(|m| m.as_str().to_string()))
                        .collect();
                    ...
                    if !interface_names.is_empty() {
                        result.insert(
                            ...
                            self.determine_naming_convention(&interface_names),
                        );
                    }
                }
            }
        ...
        for profile in analysis.file_profiles.values() {
            ...
            *brace_style_counts.entry(brace_key).or_insert(0) += 1;
            ...
            for (category, convention) in &profile.naming {
                let convention_map = naming_conventions
        ...
        for (file_path, profile) in &analysis.file_profiles {
                ...
                && profile.brace_style != BraceStyle::Unknown
            {
                ...
                inconsistencies.push(StyleInconsistency {
                    ...
                    line_number: None,
                    description: format!(
                        "Inconsistent brace style: expected {}, found {}",
                        expected, actual
                    ),
            ...
            if profile.line_metrics.over_limit_count > 0 {
                ...
                inconsistencies.push(StyleInconsistency {
                    ...
                    description: format!(
                        ...
                        profile.line_metrics.over_limit_count, LINE_LENGTH_LIMIT
                    ),
                    severity: InconsistencySeverity::Low,
                });
            ...
            if !is_small_file {
                if let (Some(global_semi), Some(file_semi)) = (
                    global.has_trailing_semicolons,
                    profile.has_trailing_semicolons,
                ) {
            ...
            if profile.trailing_whitespace_count > 0 {
                ...
                inconsistencies.push(StyleInconsistency {
                    ...
                    line_number: None,
                    description: format!(
                        "File contains {} lines with trailing whitespace",
                        profile.trailing_whitespace_count
                    ),
                    severity: InconsistencySeverity::Low,
                });
