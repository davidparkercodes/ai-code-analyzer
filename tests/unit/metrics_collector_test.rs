...
"#;
    ...
    file.write_all(py_content.as_bytes()).expect("Failed to write to file");
...
"#;
    ...
    file.write_all(js_content.as_bytes()).expect("Failed to write to file");
}
...
fn test_collect_metrics() {
    ...
    // Verify Rust metrics
    let rust_metrics = metrics.by_language.get("Rust").expect("No Rust metrics found");
    assert_eq!(rust_metrics.files, 1);
    
    // Verify Python metrics
    let py_metrics = metrics.by_language.get("Python").expect("No Python metrics found");
    assert_eq!(py_metrics.files, 1);
    
    // Verify JavaScript metrics
    let js_metrics = metrics.by_language.get("JavaScript").expect("No JavaScript metrics found");
